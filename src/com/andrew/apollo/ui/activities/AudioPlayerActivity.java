 ...
 */
public class AudioPlayerActivity extends SherlockFragmentActivity implements
		ServiceConnection, OnSeekBarChangeListener {

	// mAlbumArt Default height;
	int AAwidth, AAheight;
	...
	protected void onPause() {
		...
		if (MusicUtils.isPlaying()
				&& ApolloUtils.isApplicationSentToBackground(this)) {
	...
	private void initPlaybackControls() {
		...
		AAwidth = mAlbumArt.getWidth();
		AAheight = mAlbumArt.getHeight();

		mAlbumArt.setOnTouchListener(albumArtOnTouch);
	}

	/**
	 * XXX dcsms albumart on touchevent
	 */
	private OnTouchListener albumArtOnTouch = new OnTouchListener() {
		boolean swipe = false;
		int lastX;
		int meX;
		...
		public boolean onTouch(View v, MotionEvent e) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (swipe) {
					float proses = (e.getRawX() - lastX);
					meX=(int) e.getRawX();
					Log.e("SWIPE", "posX : " + proses);
					if ((e.getRawX() > lastX) && proses > 10) {
						ViewHelper.setTranslationX(mAlbumArt, proses);
						zoom(mAlbumArt, proses / 300, true);
					} else if (e.getRawX() < lastX && proses < 10) {
						ViewHelper.setTranslationX(mAlbumArt, proses);
						zoom(mAlbumArt, proses / 300, false);
					}
			...
			case MotionEvent.ACTION_CANCEL:
				if (swipe) {
					if ((e.getRawX() > lastX) && (e.getRawX() - lastX) > 100) {
						MusicUtils.next();
						setAlbumArtDefaultState();
					} else if ((e.getRawX() < lastX)
							&& (lastX - e.getRawX()) > 100) {
	...
	private void transalate(int value){
		
	}

	private void setAlbumArtDefaultState() {
		ViewHelper.setTranslationX(mAlbumArt, 0);
		zoom(mAlbumArt, 0, true);
	}

	private void zoom(View left, float positionOffset, boolean in) {

		float mScale = in ? 0.5f + (1 - 0.5f) * (1 - positionOffset)
				: 1 + 0.5f - 0.5f * (1 - positionOffset);
		ViewHelper.setPivotX(left, left.getMeasuredWidth() * 0.5f);
		ViewHelper.setPivotY(left, left.getMeasuredHeight() * 0.5f);
		ViewHelper.setScaleX(left, (float) mScale);
		ViewHelper.setScaleY(left, (float) mScale);
		ViewHelper.setAlpha(left, mScale);
	}
	...
	private void shareCurrentTrack() {
		...
		final String shareMessage = getString(R.string.now_listening_to) + " "
				...
				+ getString(R.string.hash_apollo);
